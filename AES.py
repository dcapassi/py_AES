...
AES_PARAMETERS = {128:{'subkeys':11, 'columns':4,'rounds':10},192:{'subkeys':13,'columns':6,'rounds':12},256:{'subkeys':15,'columns':8,'rounds':14}}
...
def matrix_to_string_ascii_list(matrix):
    matrix_t = transpose_matrix(matrix)
    output = []
    for row in matrix_t:
        for byte in row:
            byte_value = byte[2:]
            output.append(int(byte_value,16))
    return output
...
def encrypt(key, text):
    ...
    if len(key) == 16:
        ...
        raise ValueError("Input key must have 16, 24 or 32 characters")

    key_hexa = ''.join([c.encode('utf-8').hex() for c in key])
    key_array = create_subkey_list(key_hexa, key_size)

    text_matrix = str_to_matrix_hexa(text)
    state_matrix = xor_matrix(key_array[0],text_matrix)

    r = 1
    for round in range(1, AES_PARAMETERS[key_size]['rounds']):
    ...
    sub_bytes_matrix = matrix_subword(state_matrix)
    shift_row_matrix = shift_row_transformation(sub_bytes_matrix)
    state_matrix = xor_matrix(shift_row_matrix, key_array[AES_PARAMETERS[key_size]['rounds']])
...
def decrypt(cipher_text, key):
    ...
    state_matrix = xor_matrix(cipher_text_matrix, key_array[AES_PARAMETERS[key_size]['rounds']])

    for round in list(reversed(range(1, AES_PARAMETERS[key_size]['rounds']))):
...
def transpose_matrix_any(matrix):
    return [list(row) for row in zip(*matrix)]



def create_subkey_list(key_hex,key_size):
    columns = AES_PARAMETERS[key_size]['columns']

    hex_list = []
    for i in range(0,len(key_hex),2):
        hex_list.append('0x'+key_hex[i:i+2])
    list_of_words = vector_to_matrix(hex_list,columns)

    rcon_round = 1

    for i in range(AES_PARAMETERS[key_size]['columns'],AES_PARAMETERS[key_size]['subkeys']*4):
        ##AES-256
        last_word = list_of_words[-1]
        if key_size==256:
            if i % 8 == 0:
                vector_last_row_shifted = shift_word(last_word,1,'left')
                vector_subword_output = vector_subword(vector_last_row_shifted)
                vector_xor_round_output = vector_xor_round(vector_subword_output,rcon_round)
                rcon_round = rcon_round + 1
                new_word = vectors_xor(vector_xor_round_output,list_of_words[i-8])
                list_of_words.append(new_word)
            elif i % 8 == 4:
                new_word = vector_subword(last_word)
                new_word = vectors_xor(new_word,list_of_words[i-8])
                list_of_words.append(new_word)
            else:
                new_word = vectors_xor(last_word,list_of_words[i-8])
                list_of_words.append(new_word)
        ...
        if key_size==192:
            if i % 6 == 0:
                ...
                vector_xor_round_output = vector_xor_round(vector_subword_output,rcon_round)
                rcon_round = rcon_round + 1
                new_word = vectors_xor(vector_xor_round_output,list_of_words[i-6])
                list_of_words.append(new_word)
            else:
                new_word = vectors_xor(last_word,list_of_words[i-6])
                list_of_words.append(new_word)
        ...
        if key_size==128:
            if i % 4 == 0:
                ...
                vector_xor_round_output = vector_xor_round(vector_subword_output,rcon_round)
                rcon_round = rcon_round + 1
                new_word = vectors_xor(vector_xor_round_output,list_of_words[i-4])
                list_of_words.append(new_word)
            else:
                new_word = vectors_xor(last_word,list_of_words[i-4])
                list_of_words.append(new_word)

    list_of_subkeys = []
    for i in range(0, len(list_of_words), 4):
        column_set = [list_of_words[i], list_of_words[i + 1], list_of_words[i + 2], list_of_words[i + 3]]
        list_of_subkeys.append(transpose_matrix(column_set))
        
    return list_of_subkeys
